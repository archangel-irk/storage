...
id=newDoc._id.toString(),this.documents[id]=newDoc}return _storageWillMutate||this.storageHasMutated(),this.documents[id]},remove:function(document){return delete this.documents[document._id||document]},find:function(conditions){return _.where(this.documents,conditions)},findById:function(_id){return this.documents[_id]},findByIdAndRemove:function(_id){return this.remove(this.findById(_id)),this},findByIdAndUpdate:function(_id,path,value){return this.update(this.findById(_id),path,value)},findOne:function(conditions){return _.findWhere(this.documents,conditions)},findOneAndRemove:function(conditions){return this.remove(this.findOne(conditions)),this},findOneAndUpdate:function(conditions,path,value){return this.update(this.findOne(conditions),path,value)},update:function(document,path,value){var doc=this.documents[document._id||document];return null==doc?(console.warn("storage::update: Document is not found."),!1):doc.set(path,value)},storageHasMutated:function(){this.array=_.toArray(this.documents)},updateIdLink:function(doc){var id=doc._id.toString(),oldId=_.findKey(this.documents,{_id:doc._id});if(!oldId)throw new TypeError("Не найден документ для обновления ссылки по этому _id: "+id);delete this.documents[oldId],this.documents[id]=doc}},/*!
	 * Module exports.
	 */
module.exports=Collection},function(module,exports,__webpack_require__){"use strict";function Schema(name,baseSchema,obj,options){if(!(this instanceof Schema))return new Schema(name,baseSchema,obj,options);"string"==typeof name?(this.name=name,schemas[name]=this):(options=obj,obj=baseSchema,baseSchema=name,name=void 0),baseSchema instanceof Schema||(options=obj,obj=baseSchema,baseSchema=void 0),this.paths={},this.subpaths={},this.virtuals={},this.nested={},this.inherits={},this.callQueue=[],this.methods={},this.statics={},this.tree={},this._requiredpaths=void 0,this.discriminatorMapping=void 0,this.options=this.defaultOptions(options),baseSchema instanceof Schema&&baseSchema.discriminator(name,this),obj&&this.add(obj);var _idSubDoc=obj&&obj._id&&_.isObject(obj._id),auto_id=!this.paths._id&&!this.options.noId&&this.options._id&&!_idSubDoc;auto_id&&this.add({_id:{type:Schema.ObjectId,auto:!0}});var autoid=!this.paths.id&&this.options.id;autoid&&this.virtual("id").get(idGetter);var timestamps=this.options.timestamps;if(timestamps){var createdAt=timestamps.createdAt||"createdAt",updatedAt=timestamps.updatedAt||"updatedAt",schemaAdditions={};schemaAdditions[updatedAt]=Date,this.paths[createdAt]||(schemaAdditions[createdAt]=Date),this.add(schemaAdditions)}}/*!
...
exports.random=function(){return Math.random().toString().substr(3)},/*!
	 ...
	 * @api private
	 */
exports.isStorageObject=function(v){return Document||(Document=__webpack_require__(14)),v instanceof Document||v&&v.isStorageArray};var isStorageObject=exports.isStorageObject;/*!
...
function K(v){return v}exports.get=function(path,o,special,map){var lookup;"function"==typeof special&&(special.length<2?(map=special,special=void 0):(lookup=special,special=void 0)),map||(map=K);var parts="string"==typeof path?path.split("."):path;if(!Array.isArray(parts))throw new TypeError("Invalid `path`. Must be either string or array");for(var part,obj=o,i=0;i<parts.length;++i){if(part=parts[i],Array.isArray(obj)&&!/^\d+$/.test(part)){var paths=parts.slice(i);return obj.map(function(item){return item?exports.get(paths,item,special||lookup,map):map(void 0)})}if(obj=lookup?lookup(obj,part):special&&obj[special]?obj[special][part]:obj[part],!obj)return map(obj)}return map(obj)},exports.set=function(path,val,o,special,map,_copying){var lookup;"function"==typeof special&&(special.length<2?(map=special,special=void 0):(lookup=special,special=void 0)),map||(map=K);var parts="string"==typeof path?path.split("."):path;if(!Array.isArray(parts))throw new TypeError("Invalid `path`. Must be either string or array");if(null!=o){for(var part,copy=_copying||/\$/.test(path),obj=o,i=0,len=parts.length-1;len>i;++i)if(part=parts[i],"$"!==part){if(Array.isArray(obj)&&!/^\d+$/.test(part)){var paths=parts.slice(i);if(!copy&&Array.isArray(val))for(var j=0;j<obj.length&&j<val.length;++j)exports.set(paths,val[j],obj[j],special||lookup,map,copy);else for(var j=0;j<obj.length;++j)exports.set(paths,val,obj[j],special||lookup,map,copy);return}if(obj=lookup?lookup(obj,part):special&&obj[special]?obj[special][part]:obj[part],!obj)return}else if(i==len-1)break;if(part=parts[len],special&&obj[special]&&(obj=obj[special]),Array.isArray(obj)&&!/^\d+$/.test(part))if(!copy&&Array.isArray(val))for(var item,j=0;j<obj.length&&j<val.length;++j)item=obj[j],item&&(lookup?lookup(item,part,map(val[j])):(item[special]&&(item=item[special]),item[part]=map(val[j])));else for(var j=0;j<obj.length;++j)item=obj[j],item&&(lookup?lookup(item,part,map(val)):(item[special]&&(item=item[special]),item[part]=map(val)));else lookup?lookup(obj,part,map(val)):obj[part]=map(val)}}},function(module,exports,__webpack_require__){(function(Buffer){"use strict";function Document(data,collectionName,schema,fields,init){if(!(this instanceof Document))return new Document(data,collectionName,schema,fields,init);if(this.$__=new InternalCache,this.$__.emitter=new EventEmitter,this.isNew=!0,"boolean"==typeof data&&(init=data,data=null),collectionName instanceof Schema&&(schema=collectionName,collectionName=void 0),!_.isObject(schema)||schema instanceof Schema||(schema=new Schema(schema)),data instanceof Schema?(schema=data,data=null,schema.options._id&&(data={_id:new ObjectId})):(schema=this.schema||schema,schema&&!this.schema&&schema.options._id&&(data=data||{},void 0===data._id&&(data._id=new ObjectId))),!schema)throw new StorageError.MissingSchemaError;"boolean"==typeof collectionName&&(init=collectionName,collectionName=void 0),"boolean"==typeof fields?(this.$__.strictMode=fields,fields=void 0):(this.$__.strictMode=schema.options&&schema.options.strict,this.$__.selected=fields),this.schema=schema,collectionName&&(this.collection=window.storage[collectionName],this.collectionName=collectionName);for(var required=schema.requiredPaths(),i=0;i<required.length;++i)this.$__.activePaths.require(required[i]);if(this.$__setSchema(schema),this.$__.emitter.setMaxListeners(0),this._doc=this.$__buildDoc(data,init),init?this.init(data):data&&this.set(data,void 0,!0),schema.options.strict===!1&&data){var self=this,keys=Object.keys(this._doc);keys.forEach(function(key){key in schema.tree||defineProp(self,key,null,self)})}for(var i in schema.methods)"function"==typeof schema.methods[i]?this[i]=schema.methods[i]:!function(_i){Object.defineProperty(this,_i,{get:function(){var h={};for(var k in schema.methods[_i])h[k]=schema.methods[_i][k].bind(this);return h}})}(i);for(var i in schema.statics)Object.defineProperty(this,i,{value:schema.statics[i],writable:!1})}/*!
	 ...
	 * @api private
	 */
function init(self,obj,doc,prefix){prefix=prefix||"";for(var schema,path,i,keys=Object.keys(obj),len=keys.length;len--;)if(i=keys[len],path=prefix+i,schema=self.schema.path(path),schema||!_.isPlainObject(obj[i])||obj[i].constructor&&"Object"!==utils.getFunctionName(obj[i].constructor)){if(null===obj[i])doc[i]=null;else if(void 0!==obj[i]){if(schema)try{doc[i]=schema.cast(obj[i],self,!0)}catch(e){self.invalidate(e.path,new ValidatorError({path:e.path,message:e.message,type:"cast",value:e.value}))}else doc[i]=obj[i];self.adapterHooks.documentSetInitialValue.call(self,self,path,doc[i])}self.isModified(path)||self.$__.activePaths.init(path)}else doc[i]||(doc[i]={}),init(self,obj[i],doc[i],path+".")}/*!
	 * Compiles schemas.
...
function compile(self,tree,proto,prefix){for(var limb,key,keys=Object.keys(tree),i=keys.length;i--;)key=keys[i],limb=tree[key],defineProp(self,key,"Object"!==utils.getFunctionName(limb.constructor)||!Object.keys(limb).length||limb.type&&!limb.type.type?null:limb,proto,prefix,keys)}function getOwnPropertyDescriptors(object){var result={};return Object.getOwnPropertyNames(object).forEach(function(key){result[key]=Object.getOwnPropertyDescriptor(object,key),result[key].enumerable=!1}),result}/*!
	 ...
	 */
function defineProp(self,prop,subprops,prototype,prefix,keys){prefix=prefix||"";var path=(prefix?prefix+".":"")+prop;subprops?Object.defineProperty(prototype,prop,{enumerable:!0,configurable:!0,get:function(){if(this.$__.getters||(this.$__.getters={}),!this.$__.getters[path]){var nested=Object.create(Object.getPrototypeOf(this),getOwnPropertyDescriptors(this));prefix||(nested.$__.scope=this);for(var i=0,len=keys.length;len>i;++i)Object.defineProperty(nested,keys[i],{enumerable:!1,writable:!0,configurable:!0,value:void 0});nested.toObject=function(){return this.get(path)},nested.toJSON=nested.toObject,nested.$__isNested=!0,compile(self,subprops,nested,path),this.$__.getters[path]=nested}return this.$__.getters[path]},set:function(v){return v instanceof Document&&(v=v.toObject()),(this.$__.scope||this).set(path,v)}}):Object.defineProperty(prototype,prop,{enumerable:!0,configurable:!0,get:function(){return this.get.call(this.$__.scope||this,path)},set:function(v){return this.set.call(this.$__.scope||this,path,v)}}),self.adapterHooks.documentDefineProperty.call(self,self,prototype,prop,prefix,path)}/*!
...
function applyGetters(self,json,type,options){for(var path,schema=self.schema,paths=Object.keys(schema[type]),i=paths.length;i--;){path=paths[i];for(var part,parts=path.split("."),plen=parts.length,last=plen-1,branch=json,ii=0;plen>ii;++ii)part=parts[ii],ii===last?branch[part]=utils.clone(self.get(path),options):branch=branch[part]||(branch[part]={})}return json}/*!
	 * Module dependencies.
	 */
var DocumentArray,Embedded,EventEmitter=__webpack_require__(7).EventEmitter,StorageError=__webpack_require__(15),MixedSchema=__webpack_require__(21),ObjectId=__webpack_require__(11),Schema=__webpack_require__(6),ValidatorError=__webpack_require__(22).ValidatorError,Deferred=__webpack_require__(23),utils=__webpack_require__(9),clone=utils.clone,ValidationError=StorageError.ValidationError,InternalCache=__webpack_require__(24),deepEqual=utils.deepEqual;/*!
	 * Document exposes the NodeJS event emitter API, so you can use
	 * `on`, `once`, etc.
	 */
["on","once","emit","listeners","removeListener","setMaxListeners","removeAllListeners","addListener"].forEach(function(emitterFn){Document.prototype[emitterFn]=function(){return this.$__.emitter[emitterFn].apply(this.$__.emitter,arguments)}}),Document.prototype.schema=null,Document.prototype.isNew=null,Document.prototype.id=null,Document.prototype.errors=null,Document.prototype.adapterHooks={documentDefineProperty:_.noop,documentSetInitialValue:_.noop,documentGetValue:_.noop,documentSetValue:_.noop},Document.prototype.$__buildDoc=function(obj,skipId){for(var doc={},self=this,paths=Object.keys(this.schema.paths),plen=paths.length,ii=0;plen>ii;++ii){var p=paths[ii];if("_id"===p){if(skipId)continue;if(obj&&"_id"in obj)continue}for(var type=this.schema.paths[p],path=p.split("."),len=path.length,last=len-1,doc_=doc,i=0;len>i;++i){var defaultVal,piece=path[i];i===last?(defaultVal=type.getDefault(self,!0),"undefined"!=typeof defaultVal&&(doc_[piece]=defaultVal,self.$__.activePaths["default"](p))):doc_=doc_[piece]||(doc_[piece]={})}}return doc},Document.prototype.init=function(data){/*!
	  ...
	  */
return this.isNew=!1,init(this,data,this._doc),this},Document.prototype.set=function(path,val,type,options){type&&"Object"===utils.getFunctionName(type.constructor)&&(options=type,type=void 0);var adhocs,merge=options&&options.merge,adhoc=type&&!0!==type,constructing=!0===type,strict=options&&"strict"in options?options.strict:this.$__.strictMode;if(adhoc&&(adhocs=this.$__.adhocPaths||(this.$__.adhocPaths={}),adhocs[path]=Schema.interpretAsType(path,type)),"string"!=typeof path){if(null!==path&&void 0!==path){var prefix=val?val+".":"";path instanceof Document&&(path=path._doc);for(var pathtype,key,keys=Object.keys(path),i=keys.length;i--;){key=keys[i];var pathName=prefix+key;if(pathtype=this.schema.pathType(pathName),null==path[key]||!_.isPlainObject(path[key])||path[key].constructor&&"Object"!==utils.getFunctionName(path[key].constructor)||"virtual"===pathtype||"real"===pathtype||this.$__path(pathName)instanceof MixedSchema||this.schema.paths[pathName]&&this.schema.paths[pathName].options&&this.schema.paths[pathName].options.ref)if(strict){if("real"===pathtype||"virtual"===pathtype)this.set(prefix+key,path[key],constructing);else if("throw"===strict)throw new Error("Field `"+key+"` is not in schema.")}else void 0!==path[key]&&this.set(prefix+key,path[key],constructing);else this.set(path[key],pathName,constructing)}return this}var _temp=path;path=val,val=_temp}var pathType=this.schema.pathType(path);if("nested"===pathType&&val&&_.isPlainObject(val)&&(!val.constructor||"Object"===utils.getFunctionName(val.constructor)))return merge||this.setValue(path,null),this.set(val,path,constructing),this;var schema,subpath,parts=path.split(".");if("adhocOrUndefined"===pathType&&strict){for(var mixed,i=0;i<parts.length;++i)if(subpath=parts.slice(0,i+1).join("."),schema=this.schema.path(subpath),schema instanceof MixedSchema){mixed=!0;break}if(!mixed){if("throw"===strict)throw new Error("Field `"+path+"` is not in schema.");return this}}else{if("virtual"===pathType)return schema=this.schema.virtualpath(path),schema.applySetters(val,this),this;schema=this.$__path(path)}var pathToMark;if(parts.length<=1)pathToMark=path;else{for(i=0;i<parts.length;++i)if(subpath=parts.slice(0,i+1).join("."),this.isDirectModified(subpath)||null===this.get(subpath)){pathToMark=subpath;break}pathToMark||(pathToMark=path)}var priorVal=constructing?void 0:this.getValue(path);if(!schema||void 0===val)return this.$__set(pathToMark,path,constructing,parts,schema,val,priorVal),this;var shouldSet=!0;try{var didPopulate=!1;schema.options&&schema.options.ref&&val instanceof Document&&schema.options.ref===val.schema.name&&(this.populated(path,val._id),didPopulate=!0),val=schema.applySetters(val,this,!1,priorVal),!didPopulate&&this.$__.populated&&delete this.$__.populated[path],this.$markValid(path)}catch(e){var reason;e instanceof StorageError.CastError||(reason=e),this.invalidate(path,new StorageError.CastError(schema.instance,val,path,reason)),shouldSet=!1}return shouldSet&&this.$__set(pathToMark,path,constructing,parts,schema,val,priorVal),this},Document.prototype.$__shouldModify=function(pathToMark,path,constructing,parts,schema,val,priorVal){return this.isNew?!0:void 0!==val||this.isSelected(path)?void 0===val&&path in this.$__.activePaths.states["default"]?!1:utils.deepEqual(val,priorVal||this.get(path))?!1:!0:!0},Document.prototype.$__set=function(pathToMark,path,constructing,parts,schema,val,priorVal){var shouldModify=this.$__shouldModify.apply(this,arguments);shouldModify&&this.markModified(pathToMark,val);for(var obj=this._doc,i=0,l=parts.length;l>i;i++){var next=i+1,last=next===l;last?(obj[parts[i]]=val,this.adapterHooks.documentSetValue.call(this,this,path,val)):obj=obj[parts[i]]&&"Object"===utils.getFunctionName(obj[parts[i]].constructor)?obj[parts[i]]:obj[parts[i]]&&"EmbeddedDocument"===utils.getFunctionName(obj[parts[i]].constructor)?obj[parts[i]]:obj[parts[i]]&&Array.isArray(obj[parts[i]])?obj[parts[i]]:obj[parts[i]]={}}},Document.prototype.getValue=function(path){return utils.getValue(path,this._doc)},Document.prototype.setValue=function(path,value){return utils.setValue(path,value,this._doc),this},Document.prototype.get=function(path,type){var adhocs;type&&(adhocs=this.$__.adhocPaths||(this.$__.adhocPaths={}),adhocs[path]=Schema.interpretAsType(path,type));for(var schema=this.$__path(path)||this.schema.virtualpath(path),pieces=path.split("."),obj=this._doc,i=0,l=pieces.length;l>i;i++)obj=void 0===obj||null===obj?void 0:obj[pieces[i]];return schema&&(obj=schema.applyGetters(obj,this)),this.adapterHooks.documentGetValue.call(this,this,path),obj},Document.prototype.$__path=function(path){var adhocs=this.$__.adhocPaths,adhocType=adhocs&&adhocs[path];return adhocType?adhocType:this.schema.path(path)},Document.prototype.markModified=function(path){this.$__.activePaths.modify(path)},Document.prototype.modifiedPaths=function(){var directModifiedPaths=Object.keys(this.$__.activePaths.states.modify);return directModifiedPaths.reduce(function(list,path){var parts=path.split(".");return list.concat(parts.reduce(function(chains,part,i){return chains.concat(parts.slice(0,i).concat(part).join("."))},[]))},[])},Document.prototype.isModified=function(path){return path?!!~this.modifiedPaths().indexOf(path):this.$__.activePaths.some("modify")},Document.prototype.isDirectModified=function(path){return path in this.$__.activePaths.states.modify},Document.prototype.isInit=function(path){return path in this.$__.activePaths.states.init},Document.prototype.isSelected=function(path){if(this.$__.selected){if("_id"===path)return 0!==this.$__.selected._id;var cur,paths=Object.keys(this.$__.selected),i=paths.length,inclusive=!1;if(1===i&&"_id"===paths[0])return 0===this.$__.selected._id;for(;i--;)if(cur=paths[i],"_id"!==cur){inclusive=!!this.$__.selected[cur];break}if(path in this.$__.selected)return inclusive;i=paths.length;for(var pathDot=path+".";i--;)if(cur=paths[i],"_id"!==cur){if(0===cur.indexOf(pathDot))return inclusive;if(0===pathDot.indexOf(cur+"."))return inclusive}return!inclusive}return!0},Document.prototype.validate=function(cb){function validatePath(path){validating[path]||(validating[path]=!0,total++,utils.setImmediate(function(){var p=self.schema.path(path);if(!p)return--total||complete();var val=self.getValue(path);p.doValidate(val,function(err){err&&self.invalidate(path,err,void 0),--total||complete()},self)}))}function complete(){var err=self.$__.validationError;self.$__.validationError=void 0,cb&&cb(err)}var self=this,paths=Object.keys(this.$__.activePaths.states.require).filter(function(path){return self.isSelected(path)||self.isModified(path)?!0:!1});if(paths=paths.concat(Object.keys(this.$__.activePaths.states.init)),paths=paths.concat(Object.keys(this.$__.activePaths.states.modify)),paths=paths.concat(Object.keys(this.$__.activePaths.states["default"])),0===paths.length)return complete(),this;for(var validating={},total=0,i=0;i<paths.length;++i){var path=paths[i],val=self.getValue(path);if(val instanceof Array&&!Buffer.isBuffer(val)&&!val.isStorageDocumentArray)for(var numElements=val.length,j=0;numElements>j;++j)paths.push(path+"."+j)}return paths.forEach(validatePath),this},Document.prototype.validateSync=function(){var self=this,paths=Object.keys(this.$__.activePaths.states.require).filter(function(path){return self.isSelected(path)||self.isModified(path)?!0:!1});paths=paths.concat(Object.keys(this.$__.activePaths.states.init)),paths=paths.concat(Object.keys(this.$__.activePaths.states.modify)),paths=paths.concat(Object.keys(this.$__.activePaths.states["default"]));for(var validating={},i=0;i<paths.length;++i){var path=paths[i],val=self.getValue(path);if(val instanceof Array&&!Buffer.isBuffer(val)&&!val.isStorageDocumentArray)for(var numElements=val.length,j=0;numElements>j;++j)paths.push(path+"."+j)}paths.forEach(function(path){if(!validating[path]){validating[path]=!0;var p=self.schema.path(path);if(p&&self.$isValid(path)){var val=self.getValue(path),err=p.doValidateSync(val,self);err&&self.invalidate(path,err,void 0)}}});var err=self.$__.validationError;if(self.$__.validationError=void 0,self.emit("validate",self),err)for(var key in err.errors)err.errors[key]instanceof StorageError.CastError&&self.invalidate(key,err.errors[key]);return err},Document.prototype.invalidate=function(path,errorMsg,value){this.$__.validationError||(this.$__.validationError=new ValidationError(this)),this.$__.validationError.errors[path]||(errorMsg&&"string"!=typeof errorMsg||(errorMsg=new ValidatorError({path:path,message:errorMsg,type:"user defined",value:value})),this.$__.validationError!=errorMsg&&(this.$__.validationError.errors[path]=errorMsg))},Document.prototype.$markValid=function(path){this.$__.validationError&&this.$__.validationError.errors[path]&&(delete this.$__.validationError.errors[path],0===Object.keys(this.$__.validationError.errors).length&&(this.$__.validationError=null))},Document.prototype.$isValid=function(path){return!this.$__.validationError||!this.$__.validationError.errors[path]},Document.prototype.$__reset=function(){var self=this;return this.$__.activePaths.map("init","modify",function(i){return self.getValue(i)}).filter(function(val){return val&&val.isStorageDocumentArray&&val.length}).forEach(function(array){for(var i=array.length;i--;){var doc=array[i];doc&&doc.$__reset()}}),this.$__.activePaths.clear("modify"),this.$__.validationError=void 0,this.errors=void 0,this.schema.requiredPaths().forEach(function(path){self.$__.activePaths.require(path)}),this},Document.prototype.$__dirty=function(){var self=this,all=this.$__.activePaths.map("modify",function(path){return{path:path,value:self.getValue(path),schema:self.$__path(path)}});all.sort(function(a,b){return a.path<b.path?-1:a.path>b.path?1:0});var lastPath,top,minimal=[];return all.forEach(function(item){lastPath=item.path+".",minimal.push(item),top=item}),top=lastPath=null,minimal},Document.prototype.$__setSchema=function(schema){this.schema=schema,compile(this,schema.tree,this)},Document.prototype.$__getAllSubdocs=function(){function docReducer(seed,path){var val=this[path];return val instanceof Embedded&&seed.push(val),val instanceof DocumentArray&&val.forEach(function(doc){doc&&doc._doc&&(doc instanceof Embedded&&seed.push(doc),seed=Object.keys(doc._doc).reduce(docReducer.bind(doc._doc),seed))}),seed}return DocumentArray||(DocumentArray=__webpack_require__(26)),Embedded=Embedded||__webpack_require__(28),Object.keys(this._doc).reduce(docReducer.bind(this),[])},Document.prototype.$__presaveValidate=function(){var docs=this.$__getArrayPathsToValidate(),e2=docs.map(function(doc){return doc.$__presaveValidate()}),e1=[this.$__.saveError].concat(e2),err=e1.filter(function(x){return x})[0];return this.$__.saveError=null,err},Document.prototype.$__getArrayPathsToValidate=function(){return DocumentArray||(DocumentArray=__webpack_require__(26)),this.$__.activePaths.map("init","modify",function(i){return this.getValue(i)}.bind(this)).filter(function(val){return val&&val instanceof DocumentArray&&val.length}).reduce(function(seed,array){return seed.concat(array)},[]).filter(function(doc){return doc})},Document.prototype.$__error=function(err){return this.$__.saveError=err,this},Document.prototype.$__delta=function(){for(var dirty=this.$__dirty(),delta={},len=dirty.length,d=0;len>d;++d){var data=dirty[d],value=data.value;value=utils.clone(value,{depopulate:1}),delta[data.path]=value}return delta},Document.prototype.$__handleSave=function(){var resource;this.collection&&(resource=this.collection.api);var innerPromise=new Deferred;if(this.isNew){var toObjectOptions={};this.schema.options.toObject&&this.schema.options.toObject.retainKeyOrder&&(toObjectOptions.retainKeyOrder=!0),toObjectOptions.depopulate=1,toObjectOptions._skipDepopulateTopLevel=!0,toObjectOptions.transform=!1;var obj=this.toObject(toObjectOptions);if((obj||{}).hasOwnProperty("_id")===!1)return innerPromise.reject(new Error("document must have an _id before saving")),innerPromise;resource?resource.create(obj).always(innerPromise.resolve):innerPromise.resolve(obj),this.$__reset(),this.isNew=!1,this.emit("isNew",!1),this.$__.inserting=!0}else{this.$__.inserting=!1;var delta=this.$__delta();_.isEmpty(delta)?(this.$__reset(),innerPromise.resolve(this)):(this.$__reset(),resource?resource(this.id).update(delta).always(innerPromise.resolve):innerPromise.resolve(delta)),this.emit("isNew",!1)}return innerPromise},Document.prototype.save=function(done){var self=this,finalPromise=(new Deferred).done(done);if(!this.collection)return finalPromise.reject(arguments),console.error("Document.save api handle is not implemented."),finalPromise;var preSaveErr=self.$__presaveValidate();if(preSaveErr)return finalPromise.reject(preSaveErr),finalPromise;var p0=new Deferred;self.validate(function(err){err?(p0.reject(err),finalPromise.reject(err)):p0.resolve()});var subDocs=self.$__getAllSubdocs(),whenCond=subDocs.map(function(d){return d.save()});whenCond.push(p0);var p1=Deferred.when.apply(Deferred,whenCond);return p1.fail(function(err){self.$__.inserting&&(self.isNew=!0,self.emit("isNew",!0)),finalPromise.reject(err)}),p1.done(function(){self.$__handleSave().done(function(){self.emit("save",self),finalPromise.resolve.apply(finalPromise,arguments)}).fail(function(){finalPromise.reject.apply(finalPromise,arguments)})}),finalPromise},Document.prototype.$toObject=function(options,json){var defaultOptions={transform:!0,json:json};if(options&&options.depopulate&&!options._skipDepopulateTopLevel&&this.$__.wasPopulated)return clone(this._id,options);options&&options._skipDepopulateTopLevel&&(options._skipDepopulateTopLevel=!1),(!options||"Object"!=utils.getFunctionName(options.constructor)||options&&options._useSchemaOptions)&&(json?(options=this.schema.options.toJSON?clone(this.schema.options.toJSON):{},options.json=!0,options._useSchemaOptions=!0):(options=this.schema.options.toObject?clone(this.schema.options.toObject):{},options.json=!1,options._useSchemaOptions=!0));for(var key in defaultOptions)void 0===options[key]&&(options[key]=defaultOptions[key]);"minimize"in options||(options.minimize=this.schema.options.minimize);var originalTransform=options.transform,ret=clone(this._doc,options)||{};(options.virtuals||options.getters&&!1!==options.virtuals)&&applyGetters(this,ret,"virtuals",options),options.getters&&(applyGetters(this,ret,"paths",options),options.minimize&&(ret=minimize(ret)||{})),options.versionKey===!1&&this.schema.options.versionKey&&delete ret[this.schema.options.versionKey];var transform=options.transform;if(!0===transform||this.schema.options.toObject&&transform){var opts=options.json?this.schema.options.toJSON:this.schema.options.toObject;opts&&(transform="function"==typeof options.transform?options.transform:opts.transform)}else options.transform=originalTransform;if("function"==typeof transform){var xformed=transform(this,ret,options);"undefined"!=typeof xformed&&(ret=xformed)}return ret},Document.prototype.toObject=function(options){return this.$toObject(options)},Document.prototype.toJSON=function(options){return this.$toObject(options,!0)},Document.prototype.equals=function(doc){var tid=this.get("_id"),docid=doc.get("_id");return tid||docid?tid&&tid.equals?docid?tid.equals(docid):!1:tid===docid:deepEqual(this,doc)},Document.prototype.populated=function(path,val,options){if(null==val){if(!this.$__.populated)return void 0;var v=this.$__.populated[path];return v?v.value:void 0}return!0===val?this.$__.populated?this.$__.populated[path]:void 0:(this.$__.populated||(this.$__.populated={}),this.$__.populated[path]={value:val,options:options},val)},Document.prototype.$__fullPath=function(path){return path||""},Document.prototype.remove=function(){return this.collection?this.collection.remove(this):delete this},Document.prototype.empty=function(){for(var doc=this,self=this,paths=Object.keys(this.schema.paths),plen=paths.length,ii=0;plen>ii;++ii){var p=paths[ii];if("_id"!=p)for(var type=this.schema.paths[p],path=p.split("."),len=path.length,last=len-1,doc_=doc,i=0;len>i;++i){var defaultVal,piece=path[i];i===last?(defaultVal=type.getDefault(self,!0),doc_[piece]=defaultVal||void 0,self.$__.activePaths["default"](p)):doc_=doc_[piece]||(doc_[piece]={})}}},/*!
	 * Module exports.
	 */
Document.ValidationError=ValidationError,module.exports=Document}).call(exports,__webpack_require__(1).Buffer)},function(module,exports,__webpack_require__){"use strict";function StorageError(msg){this.message=msg,this.name="StorageError"}StorageError.prototype=new Error,/*!
	 * Module exports.
	 */
module.exports=StorageError,StorageError.messages=__webpack_require__(16),/*!
	 * Expose subclasses
	 */
StorageError.CastError=__webpack_require__(17),StorageError.ValidationError=__webpack_require__(18),StorageError.ValidatorError=__webpack_require__(19),StorageError.MissingSchemaError=__webpack_require__(20)},function(module,exports){"use strict";var msg=module.exports={};msg.general={},msg.general["default"]="Validator failed for path `{PATH}` with value `{VALUE}`",msg.general.required="Path `{PATH}` is required.",msg.Number={},msg.Number.min="Path `{PATH}` ({VALUE}) is less than minimum allowed value ({MIN}).",msg.Number.max="Path `{PATH}` ({VALUE}) is more than maximum allowed value ({MAX}).",msg.Date={},msg.Date.min="Path `{PATH}` ({VALUE}) is before minimum allowed value ({MIN}).",msg.Date.max="Path `{PATH}` ({VALUE}) is after maximum allowed value ({MAX}).",msg.String={},msg.String["enum"]="`{VALUE}` is not a valid enum value for path `{PATH}`.",msg.String.match="Path `{PATH}` is invalid ({VALUE}).",msg.String.minlength="Path `{PATH}` (`{VALUE}`) is shorter than the minimum allowed length ({MINLENGTH}).",msg.String.maxlength="Path `{PATH}` (`{VALUE}`) is longer than the maximum allowed length ({MAXLENGTH})."},function(module,exports,__webpack_require__){"use strict";function CastError(type,value,path){StorageError.call(this,"Cast to "+type+' failed for value "'+value+'" at path "'+path+'"'),this.name="CastError",this.type=type,this.value=value,this.path=path}/*!
	 * Module dependencies.
	 */
var StorageError=__webpack_require__(15);/*!
...
CastError.prototype=Object.create(StorageError.prototype),CastError.prototype.constructor=CastError,/*!
	 * exports
	 */
module.exports=CastError},function(module,exports,__webpack_require__){"use strict";function ValidationError(instance){StorageError.call(this,"Validation failed"),this.name="ValidationError",this.errors=instance.errors={},instance&&"model"===utils.getFunctionName(instance.constructor)||StorageError.call(this,"Validation failed"),this.name="ValidationError",this.errors={},instance&&(instance.errors=this.errors)}/*!
	 * Module requirements
	 */
var StorageError=__webpack_require__(15),utils=__webpack_require__(9);/*!
	 * Inherits from StorageError.
	 */
ValidationError.prototype=Object.create(StorageError.prototype),ValidationError.prototype.constructor=ValidationError,/*!
	 * Module exports
	 */
module.exports=ValidationError},function(module,exports,__webpack_require__){"use strict";function ValidatorError(properties){var msg=properties.message;msg||(msg=errorMessages.general["default"]),this.properties=properties;var message=this.formatMessage(msg,properties);StorageError.call(this,message),this.name="ValidatorError",this.type=properties.type,this.path=properties.path,this.value=properties.value}/*!
	 * Module dependencies.
	 */
var StorageError=__webpack_require__(15),errorMessages=StorageError.messages;/*!
	 * Inherits from StorageError
	 */
ValidatorError.prototype=Object.create(StorageError.prototype),ValidatorError.prototype.constructor=ValidatorError,/*!
	 * Formats error messages
	 */
ValidatorError.prototype.formatMessage=function(msg,properties){for(var propertyNames=Object.keys(properties),i=0;i<propertyNames.length;++i){var propertyName=propertyNames[i];"message"!==propertyName&&(msg=msg.replace("{"+propertyName.toUpperCase()+"}",properties[propertyName]))}return msg},/*!
	 * toString helper
	 */
ValidatorError.prototype.toString=function(){return this.message},/*!
	 * exports
	 */
module.exports=ValidatorError},function(module,exports,__webpack_require__){"use strict";/*!
...
MissingSchemaError.prototype=Object.create(StorageError.prototype),MissingSchemaError.prototype.constructor=StorageError,/*!
	 * exports
	 */
module.exports=MissingSchemaError},function(module,exports,__webpack_require__){"use strict";function Mixed(path,options){if(options&&options["default"]){var def=options["default"];Array.isArray(def)&&0===def.length?options["default"]=Array:!options.shared&&_.isPlainObject(def)&&0===Object.keys(def).length&&(options["default"]=function(){return{}})}SchemaType.call(this,path,options)}/*!
	 * Module dependencies.
	 */
...
function restorePopulatedFields(subdoc,schemaTree,value,prev){var props;_.forEach(schemaTree,function(prop,key){var curVal;prop.ref&&(props={},curVal=value[key],curVal&&oid.isValid(curVal)&&_.forEach(prev,function(prevDoc){var prevDocProp=prevDoc[key];prevDocProp instanceof Document&&prevDocProp._id.equals(curVal)&&(subdoc[key]=prevDocProp)}))})}/*!
	 ...
	 */
function scopePaths(array,fields,init){if(!init||!fields)return void 0;for(var hasKeys,key,path=array.path+".",keys=Object.keys(fields),i=keys.length,selected={};i--;)key=keys[i],0===key.indexOf(path)&&(hasKeys||(hasKeys=!0),selected[key.substring(path.length)]=fields[key]);return hasKeys&&selected||void 0}var SchemaType=__webpack_require__(22),ArrayType=__webpack_require__(35),StorageDocumentArray=__webpack_require__(26),Subdocument=__webpack_require__(28),Document=__webpack_require__(14),oid=__webpack_require__(11),utils=__webpack_require__(9);/*!
	 * Inherits from ArrayType.
	 */
DocumentArray.prototype=Object.create(ArrayType.prototype),DocumentArray.prototype.constructor=DocumentArray,DocumentArray.prototype.doValidate=function(array,fn,scope){var self=this;SchemaType.prototype.doValidate.call(this,array,function(err){if(err)return fn(err);var error,count=array&&array.length;if(!count)return fn();for(var i=0,len=count;len>i;++i){var doc=array[i];doc?!function(i){doc.validate(function(err){return err&&!error?(err.key=self.key+"."+i+"."+err.key,fn(error=err)):void(--count||fn())})}(i):--count||fn()}},scope)},DocumentArray.prototype.doValidateSync=function(array,scope){var schemaTypeError=SchemaType.prototype.doValidateSync.call(this,array,scope);if(schemaTypeError)return schemaTypeError;var count=array&&array.length,resultError=null;if(count){for(var i=0,len=count;len>i&&!resultError;++i){var doc=array[i];if(doc){var subdocValidateError=doc.validateSync();subdocValidateError&&(resultError=subdocValidateError)}}return resultError}},DocumentArray.prototype.cast=function(value,doc,init,prev){var selected,subdoc,i;if(!Array.isArray(value))return doc&&init&&doc.markModified(this.path),this.cast([value],doc,init,prev);if(prev&&approximatelyEqual(value,prev))return prev;if(!value.isStorageDocumentArray&&(value=new StorageDocumentArray(value,this.path,doc),prev&&prev._handlers))for(var key in prev._handlers)doc.removeListener(key,prev._handlers[key]);for(i=value.length;i--;)if(!(value[i]instanceof Subdocument)&&value[i])if(init)selected||(selected=scopePaths(this,doc.$__.selected,init)),subdoc=new this.casterConstructor(null,value,!0,selected),value[i]=subdoc.init(value[i]);else{try{subdoc=prev.id(value[i]._id)}catch(e){}prev&&subdoc?subdoc.set(value[i]):(subdoc=new this.casterConstructor(value[i],value),restorePopulatedFields(subdoc,this.schema.tree,value[i],prev)),value[i]=subdoc}return value},/*!
	 * Module exports.
	 */
module.exports=DocumentArray},function(module,exports,__webpack_require__){"use strict";function SchemaArray(key,cast,options){if(cast){var castOptions={};"Object"===utils.getFunctionName(cast.constructor)&&(cast.type?(castOptions=_.clone(cast),delete castOptions.type,cast=cast.type):cast=Mixed);var name="string"==typeof cast?cast:utils.getFunctionName(cast),Caster=name in Types?Types[name]:cast;this.casterConstructor=Caster,this.caster=new Caster(null,castOptions),EmbeddedDoc||(EmbeddedDoc=__webpack_require__(28)),this.caster instanceof EmbeddedDoc||(this.caster.path=key)}SchemaType.call(this,key,options);var defaultArr,fn,self=this;this.defaultValue&&(defaultArr=this.defaultValue,fn="function"==typeof defaultArr),this["default"](function(){var arr=fn?defaultArr():defaultArr||[];return new StorageArray(arr,self.path,this)})}/*!
	 * Module dependencies.
	 */
var EmbeddedDoc,SchemaType=__webpack_require__(22),CastError=SchemaType.CastError,Types={Boolean:__webpack_require__(33),Date:__webpack_require__(36),Number:__webpack_require__(32),String:__webpack_require__(31),ObjectId:__webpack_require__(29),Buffer:__webpack_require__(37)},StorageArray=__webpack_require__(27),Mixed=__webpack_require__(21),utils=__webpack_require__(9);/*!
	 * Inherits from SchemaType.
	 */
SchemaArray.prototype=Object.create(SchemaType.prototype),SchemaArray.prototype.constructor=SchemaArray,SchemaArray.prototype.checkRequired=function(value){return!(!value||!value.length)},SchemaArray.prototype.applyGetters=function(value,scope){return this.caster.options&&this.caster.options.ref?value:SchemaType.prototype.applyGetters.call(this,value,scope)},SchemaArray.prototype.cast=function(value,doc,init){if(Array.isArray(value)){if(value.isStorageArray||(value=new StorageArray(value,this.path,doc)),this.caster)try{for(var i=0,l=value.length;l>i;i++)value[i]=this.caster.cast(value[i],doc,init)}catch(e){throw new CastError(e.type,value,this.path)}return value}return doc&&init&&doc.markModified(this.path),this.cast([value],doc,init)},/*!
	 * Module exports.
	 */
module.exports=SchemaArray},function(module,exports,__webpack_require__){"use strict";function DateSchema(key,options){SchemaType.call(this,key,options)}/*!
	 * Module requirements.
	 */
var SchemaType=__webpack_require__(22),CastError=SchemaType.CastError;/*!
	 * Inherits from SchemaType.
	 */
DateSchema.prototype=Object.create(SchemaType.prototype),DateSchema.prototype.constructor=DateSchema,DateSchema.prototype.checkRequired=function(value){return value instanceof Date},DateSchema.prototype.cast=function(value){if(null==value||""===value)return value;if(value instanceof Date)return value;var date;if("undefined"!=typeof value&&(value instanceof Number||"number"==typeof value||String(value)==Number(value)?date=new Date(Number(value)):value.toString&&(date=new Date(value.toString())),"Invalid Date"!=date.toString()))return date;throw new CastError("date",value,this.path)},/*!
	 * Module exports.
	 */
module.exports=DateSchema},function(module,exports,__webpack_require__){(function(Buffer){"use strict";function SchemaBuffer(key,options){SchemaType.call(this,key,options,"Buffer")}/*!
	 * Module dependencies.
	 */
var Document,SchemaType=__webpack_require__(22),CastError=SchemaType.CastError,StorageBuffer=__webpack_require__(38).Buffer,Binary=StorageBuffer.Binary;/*!
	 * Inherits from SchemaType.
	 */
SchemaBuffer.prototype=Object.create(SchemaType.prototype),SchemaBuffer.prototype.constructor=SchemaBuffer,SchemaBuffer.prototype.checkRequired=function(value,doc){return SchemaType._isRef(this,value,doc,!0)?null!=value:!(!value||!value.length)},SchemaBuffer.prototype.cast=function(value,doc,init){var ret;if(SchemaType._isRef(this,value,doc,init)){if(null==value)return value;if(Document||(Document=__webpack_require__(14)),value instanceof Document)return value.$__.wasPopulated=!0,value;if(Buffer.isBuffer(value))return value;if(!_.isObject(value))throw new CastError("buffer",value,this.path);var path=doc.$__fullPath(this.path),owner=doc.ownerDocument?doc.ownerDocument():doc,pop=owner.populated(path,!0);return ret=new pop.options.model(value),ret.$__.wasPopulated=!0,ret}if(value&&value._id&&(value=value._id),Buffer.isBuffer(value))return value&&value.isStorageBuffer||(value=new StorageBuffer(value,[this.path,doc])),value;if(value instanceof Binary){if(ret=new StorageBuffer(value.value(!0),[this.path,doc]),"number"!=typeof value.sub_type)throw new CastError("buffer",value,this.path);return ret._subtype=value.sub_type,ret}if(null===value)return value;var type=typeof value;if("string"===type||"number"===type||Array.isArray(value))return ret=new StorageBuffer(value,[this.path,doc]);throw new CastError("buffer",value,this.path)},/*!
	 * Module exports.
	 */
module.exports=SchemaBuffer}).call(exports,__webpack_require__(1).Buffer)},function(module,exports,__webpack_require__){"use strict";/*!
	 * Module exports.
	 */
exports.Array=__webpack_require__(27),exports.Buffer=__webpack_require__(39),exports.Embedded=__webpack_require__(28),exports.DocumentArray=__webpack_require__(26),exports.ObjectId=__webpack_require__(11)},function(module,exports,__webpack_require__){(function(Buffer){"use strict";function StorageBuffer(value,encode,offset){var val,length=arguments.length;val=0===length||null===arguments[0]||void 0===arguments[0]?0:value;var encoding,path,doc;Array.isArray(encode)?(path=encode[0],doc=encode[1]):encoding=encode;var buf=new Buffer(val,encoding,offset);return _.mixin(buf,StorageBuffer.mixin),buf.isStorageBuffer=!0,Object.defineProperties(buf,{validators:{value:[]},_path:{value:path},_parent:{value:doc}}),doc&&"string"==typeof path&&Object.defineProperty(buf,"_schema",{value:doc.schema.path(path)}),buf._subtype=0,buf}/*!
	 * Module dependencies.
	 */
var Binary=__webpack_require__(40);__webpack_require__(9);/*!
	 * Inherit from Buffer.
	 */
...
Binary.SUBTYPE_USER_DEFINED=128,/*!
	 * Module exports.
	 */
module.exports=Binary,module.exports.Binary=Binary}).call(exports,__webpack_require__(1).Buffer)},function(module,exports){module.exports={name:"storage",version:"0.5.1",description:"Mongoose-like schema validation, collections and documents on browser (client-side)",author:"Constantine Melnikov <ka.melnikov@gmail.com>",license:"MIT",repository:{type:"git",url:"https://github.com/archangel-irk/storage.git"},main:"dist/storage.min.js",scripts:{test:"grunt test"},devDependencies:{grunt:"0.4.5","time-grunt":"1.2.1","grunt-contrib-jshint":"0.11.3","grunt-karma":"0.12.1","grunt-karma-coveralls":"2.5.4","grunt-webpack":"1.0.11",karma:"0.13.10","karma-coverage":"0.5.2","karma-mocha":"0.2.0","karma-chai":"0.1.0","karma-phantomjs-launcher":"0.2.1","karma-chrome-launcher":"0.2.0","karma-firefox-launcher":"0.1.6","karma-ie-launcher":"0.2.0","karma-safari-launcher":"0.1.1","karma-sauce-launcher":"0.2.14","karma-sourcemap-loader":"0.3.5","karma-webpack":"1.7.0",lodash:"2.4.1",webpack:"1.12.2","json-loader":"0.5.3","istanbul-instrumenter-loader":"0.1.3",dox:"0.8.0","highlight.js":"8.8.0",jade:"1.11.0",markdown:"0.5.0"}}}])});